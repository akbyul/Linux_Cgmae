...
int	 control_file_struct(void)
{
	...
	if (is_null(map_file))
		return (1);
	start_screen_file = set_file_struct(ft_str_join(txt_dir, "start_screen.txt"), 10, 19);
	if (is_null(start_screen_file))
		return (1);
	monster_slime_file = set_file_struct(ft_str_join(txt_dir, "monster_slime.txt"), 3, 24);
	if (is_null(monster_slime_file))
		return (1);
	new_or_save_file = set_file_struct(ft_str_join(txt_dir, "new_or_save.txt"), 10, 11);
	if (is_null(new_or_save_file))
		return (1);
	press_the_arrow_keys_file = set_file_struct(ft_str_join(txt_dir, "press_the_arrow_keys.txt"), 30, 20);
